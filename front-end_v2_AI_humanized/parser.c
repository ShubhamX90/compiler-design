 ...
 * CS F363 compiler project, BITS Pilani
 *
 * Grammar follows "Modified LL(1) Grammar.pdf" (Feb 13, 2026).
 * Notable rule changes from original spec:
 *   rule 10-a  : constructedDatatype can be just TK_RUID
 *   rule 20/21 : declaration uses one colon; global_or_not handles the second
 *   rule 25*   : singleOrRecId supports arbitrary dot chains
 *   rule 26    : funCallStmt ends with TK_SEM
 *   rule 30-a  : conditionalStmt uses <elsePart>
 *   rule 38-a  : NOT boolean uses parens
 *   rules B1-B7: arithmetic with proper precedence
 */
...
/* macros to make grammar construction less painful */
...
grammar *initializeGrammar(void) {
  ...
  /* 10-a. constructedDatatype can be record/union #id, or just #id */
  ...
  /* 13-a/b typeDefinitions and actualOrRedefined */
  ...
  /* fieldDefinitions needs at least 2 fieldDefinitions before moreFields */
  ...
  /* 17-a fieldDefinition -> type <fieldType> : fieldId ; */
  ...
  /* 20. declaration -> type <dataType> : id <global_or_not> ; */
  ...
  /* 25-e/f/c/d: singleOrRecId with optional dot chain */
  ...
  /* 26. funCallStmt -> <outputParameters> call funId with parameters
   * <inputParameters> ; */
  ...
  /*
   * 29. iterativeStmt - the spec says <stmts> but <stmts> includes a mandatory
   * return which can't appear inside a while body. Test cases confirm this, so
   * we use <stmt><otherStmts> instead.
   */
  ...
  /* same story for conditionalStmt */
  ...
  /* B1-B7: arithmetic expression with operator precedence */
  ...
  /* 39-a: var -> singleOrRecId | num | rnum */
  START_RULE(NT_VAR);
  NT(NT_SINGLEORRECID);
  END_RULE();
  START_RULE(NT_VAR);
  ...
  /* 46/47: definetypestmt -> definetype (record|union) #id as #id */
...
FirstAndFollow computeFirstAndFollowSets(grammar *G) {
  ...
  /* fixed-point iteration for FIRST (50 passes is more than enough) */
  ...
  /* fixed-point iteration for FOLLOW */
  for (int iter = 0; iter < 50; iter++) {
    int oldTotal = 0, newTotal = 0;
    for (int i = 0; i < F.followCount; i++)
...
void createParseTable(FirstAndFollow *F, table *T, grammar *G) {
  ...
  for (int ri = 0; ri < G->ruleCount; ri++) {
    ...
    /* compute FIRST(alpha) for this rule's RHS */
    FirstSet alpha;
    alpha.firstCount = 0;
    alpha.hasEpsilon = 0;
    ...
    if (alpha.hasEpsilon) {
      ...
      if (fw) {
        for (int j = 0; j < fw->followCount; j++) {
          ...
          if (b >= 0 && b < TABLE_TERMINALS && !T->entries[A][b].isValid) {
...
/* sync tokens for panic-mode error recovery */
...
/* skip lexical errors / comments, return next real token */
static tokenInfo nextMeaningfulToken(twinBuffer *tb, int *errCnt, int *synOK) {
  tokenInfo cur;
  do {
    cur = getNextToken(tb);
...
parseTree *parseInputSourceCode(char *testcaseFile, table *T, grammar *G) {
  ...
  int errCnt = 0, synOK = 1;
  tokenInfo cur = nextMeaningfulToken(tb, &errCnt, &synOK);
  int prevErrLine = -1;

  while (stack != NULL && errCnt < 60) {
    stackNode *topNode = pop(&stack);
    GrammarSymbol X = topNode->symbol;
    parseTreeNode *tn = topNode->treeNode;
    free(topNode);

    /* epsilon nodes are tree markers only, skip */
    ...
    if (X.type == SYMBOL_TERMINAL) {
      if (X.symbol.terminal == cur.tokenType) {
        ...
        cur = nextMeaningfulToken(tb, &errCnt, &synOK);
      } else {
        if (cur.lineNumber != prevErrLine) {
          fprintf(stderr,
        ...
        /* pop expected terminal, keep current input - let it match something
         * else */
      }
    } else {
      ...
      if (a >= 0 && a < TABLE_TERMINALS && T->entries[A][a].isValid) {
        ...
        /* push in reverse so leftmost is processed first */
        for (int i = nc - 1; i >= 0; i--)
          push(&stack, rule->rhs[i], children[i]);

      } else {
        if (cur.tokenType != TK_EOF && cur.lineNumber != prevErrLine) {
          fprintf(stderr,
        ...
        /* panic mode recovery:
         * if already on a sync token just discard A and continue,
         * otherwise skip input until we find one */
...
/* inorder traversal for printing parse tree */
