/*
 * driver.c  -  main driver for the compiler front-end
 * CS F363 - BITS Pilani
 *
 * Menu options:
 *   0 : exit
 *   1 : strip comments, print clean source
 *   2 : lex the file and print all tokens
 *   3 : full parse + write parse tree to file
 *   4 : time the lexer + parser
 */
...
/*
 * Option 2: print all tokens from the lexer.
 * TK_COMMENT tokens are included in the listing per Announcement 8.
 * Lexical errors are sent to stderr and also shown in the table.
 */
void optionPrintTokens(char *sourceFile) {
...
void optionTiming(char *sourceFile) {
  ...
  t_end = clock();
  t_ticks = (double)(t_end - t_start);
  t_sec = t_ticks / CLOCKS_PER_SEC;
